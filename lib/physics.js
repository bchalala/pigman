...
physics.run = function(steps, initialWorld) {
  ...
  return churchWorld_from_bodyList(world.GetBodyList()).reverse();
}

physics.compareShapes = function(initial, final) {
  for (var i=0; i < initial.length; i++) {
    var dx = initial[i].x - final[i].x
    var dy = initial[i].y - final[i].y
    if (dx < 0)
      dx = dx*-1
    if (dy < 0)
      dy = dy*-1
